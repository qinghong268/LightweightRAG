...
# --- New Prompt for Compression ---

COMPRESS_SYSTEM_PROMPT = """
你是一个专业的信息摘要和整合专家。你的任务是分析提供的多个文本片段，
去除冗余信息，将相关主题的内容合并，并生成 1-3 个高质量、简洁、连贯的摘要。
在每个摘要的末尾，必须标注出其来源的文件名和 chunk 编号，格式为 [source=文件名#chunk编号]。
""".strip()

def get_compress_prompt_template(retrieved_results: List[Dict[str, Any]]) -> List[dict]:
    """
    生成用于压缩上下文的 prompt 消息列表。
